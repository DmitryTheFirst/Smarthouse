...
namespace Smarthouse {
    public class TcpNetworkNext : INetwork {
        #region IModule
        ...
        public bool AuthServer( Stream newPartner, out string cryptModuleName, out string remoteId ) {
            ...
            var nameLength = wstream.ReadByte();
            var idLength = wstream.ReadByte();
            cryptModuleName = Encoding.UTF8.GetString( wstream.ReadBytes( nameLength ) );
        ...
        private async Task Listener() {
            while ( true ) ServerProcessPipe( await _listener.AcceptTcpClientAsync() );
        }
        ...
        private bool LoadConfigInternal() {
            try {
                ...
                if ( !int.TryParse( Cfg[ "listener" ].Attributes[ "port" ].Value, out tmp ) )
                    return false;
                cfg.Port = tmp;
                if ( !int.TryParse( Cfg[ "sender" ].Attributes[ "sender" ].Value, out tmp ) )
                    return false;
